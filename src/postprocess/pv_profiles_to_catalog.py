"""
Module for Photovoltaic (PV) Power Profile Re-gridding, Metadata Attachment, and Cataloging.

This module provides the primary function, 'catalog_pv_profiles',
which is responsible for converting raw, ungridded pv power generation and
...
1.  **Attribute Definition:** Defining extensive CF-compliant metadata for simulation details
    (WRF/CMIP6), scenario, installation characteristics (utility/distributed), and variable
...
2.  **Data Ingestion and Masking:** Reading ungridded Zarr time-series data and applying
    geographical and resource-suitability masks (e.g., excluding protected lands).
...
def catalog_pv_profiles(domain: str, sim_id: str, module: str, scen: str) -> None:
    """
    Regrids hourly photovoltaic capacity factor and generation profiles to the WRF grid
    ...
    domain_dict = {"d03": "3 km", "d02": "9 km"}
    nom_res = domain_dict[domain]
    ...
    cf_metadata = {
        ...
        "extended_description": (
            "Ratio of power generated to" + " panel nameplate capacity."
        ),
    }
    gen_metadata = {
    ...
    dist_attrs = {
        "photovoltaic_installation_type": "Distributed",
        "panel_array_type": "Fixed roof mount",
        "panel_azimuth": "180 deg (south-facing)",
        "panel_dc_ac_ratio": "1.3",
        "panel_ground_coverage_ratio": "0.4",
        "panel_inv_eff": "96%",
        "panel_module_type": "Standard",
        "panel_nameplate_capacity": "0.01 MW",
        "panel_tilt": "20.0 deg",
    }
    mod_md_dict = {"utility": util_attrs, "distributed": dist_attrs}
    ds_metadata = {
        "Conventions": "CF-1.8",
    ...
    # ---- load in some reference files we need ----
    ...
    mask_gen = meta.loc[(meta["landmask"] == 1) & (meta["fed_state_protected"] != 1)]
    if domain == "d03":
        idx_to_mask = pd.read_csv("/data/pv_d03_urban_remove.csv")["0"]

    filepath_zarr = f"s3://{bucket}/era/tmp/ungridded/PVWattsv8/{domain}/{module}/{sim_id}/pv_{module}_{sim_id}_"
    filepaths_zarr = [filepath_zarr + f"{y}.zarr/" for y in sim_years]
    pv_ds = xr.open_mfdataset(
        filepaths_zarr,
        engine="zarr",
        consolidated=True,
    ...
    pv_ds = pv_ds[["cf_profile", "gen_profile", "time_index"]]
    pv_ds["time"] = [
        datetime.strptime(s[:-6].decode(), "%Y-%m-%d %H:%M:%S")
        for s in pv_ds.time_index.values
    ]

    if module == "utility":
        mask = mask_gen.loc[
            (mask_gen["wind_slope_over_20p"] != 1)
            & (mask_gen["pv_land_cover_urban_high"] != 1)
        ]
    else:
        mask = mask_gen
        fix_mask = False

    pv_ds = pv_ds.assign_coords({"gid": mask.index})
    if fix_mask:
        pv_ds = pv_ds.drop_sel(gid=idx_to_mask)

    pv_ds = pv_ds.drop_vars(["time_index"])

    ix_ds = meta.to_xarray()  # need indices
    ix_ds = ix_ds.assign_coords({"time": pv_ds["time"]})
    ix_ds = ix_ds.rename({"index": "gid"})

    # do the reindexing
    pv_ds = pv_ds.reindex_like(ix_ds)
    print("reindexed to ix_ds")
    # change index name and assign coords
    pv_ds = pv_ds.rename_dims({"gid": "location"}).drop_indexes("gid")
    pv_ds = pv_ds.assign_coords({"location": template_ds.indexes["location"]})
    pv_ds = pv_ds.chunk(chunks={"time": 8760, "location": 87 * 42})
    pv_ds = pv_ds.drop_vars("gid")
    pv_ds = pv_ds.unstack()
    # pv_ds = pv_ds.chunk(chunks={"time":8760, "y":87, "x":42})
    pv_ds = pv_ds.assign_coords({"lon": lons, "lat": lats})
    out_ds = pv_ds.chunk(chunks={"time": 8760, "y": 87, "x": 42})
    ...
    out_ds = out_ds.drop(["member_id", "lakemask"])
    ...
    for var in ["cf", "gen"]:
        out_ds[var] = out_ds[var].astype(np.float32)
        out_da = out_ds[var].to_dataset()
        print(out_da)
        out_da[var].attrs = var_attr_dict[var]
        out_da.attrs = ds_metadata
        outpath = f"era/pv_{module}/{sim_id.lower()}/{scen}/1hr/{var}/"
        outname = f"{domain}"
...
if __name__ == "__main__":
    ...
    scen = str(sys.argv[4])
    catalog_pv_profiles(domain, sim_id, module, scen)
